...
void SredArifm(int[,] matrix) // среднее арифметическое столбца массива
{
    for (int j = 0; j < matrix.GetLength(1); j++)
    {
        ...
        for (int i = 0; i < matrix.GetLength(0); i++)
        {
            sum += matrix[i, j]; // суммирование
        }
        Console.WriteLine($"среднее арифметическое {j+1} столбца: {sum / matrix.GetLength(0):f2}"); // вывод среднего арифметического столбца массива
    }
}
